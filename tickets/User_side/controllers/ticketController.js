...
exports.createTicket = async (req, res) => {
    try {
        ...
        if (admin) {
            console.log(`Automatically assigning ticket ${result.insertId} to admin ${admin.username} (ID: ${admin.id})`);
            
            try {
                // Create assignment record in ticket_assignments table
                const [assignmentResult] = await db.query(
                    'INSERT INTO ticket_assignments (ticket_id, admin_id, assigned_by, assigned_at) VALUES (?, ?, ?, NOW())',
                    [result.insertId, admin.id, admin.id]
                );
                console.log('Ticket assignment record created:', assignmentResult);

                // Update ticket assignment in tickets table
                const [updateResult] = await db.query(
                    'UPDATE tickets SET assigned = "Yes", assigned_admin_id = ? WHERE id = ?',
                    [admin.id, result.insertId]
                );
                console.log('Ticket updated with admin assignment:', updateResult);

                console.log(`Ticket ${result.insertId} successfully assigned to admin ${admin.username}`);
            } catch (assignmentError) {
                console.error('Error assigning ticket:', assignmentError);
