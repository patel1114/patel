...
exports.createTicket = async (req, res) => {
    try {
        ...
        if (!req.session || !req.session.email) {
            return res.status(401).json({ error: 'Please login first to create a ticket' });
        }
        ...
        const emailSent = await sendTicketEmail(email, fullName, ticketId, pdfPath);
        console.log("Email sent:", emailSent);

        // Find and assign admin based on location
        const admin = await assignAdmin(location);
        if (admin) {
            await db.query("UPDATE tickets SET assigned = ? WHERE ticketId = ?", [admin.email, ticketId]);
            console.log("Ticket assigned to admin:", admin.email);
        }
